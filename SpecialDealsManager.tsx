...
export const SpecialDealsManager: React.FC<SpecialDealsManagerProps> = ({
  ...
  const [formSizeSmallPrice, setFormSizeSmallPrice] = useState<number | ''>(1200);
  const [formSizeMediumPrice, setFormSizeMediumPrice] = useState<number | ''>(1650);
  const [formSizeLargePrice, setFormSizeLargePrice] = useState<number | ''>(2100);
  ...
  const openCreateModal = () => {
    ...
    setFormSizeSmallPrice(1200);
    setFormSizeMediumPrice(1650);
    setFormSizeLargePrice(2100);
    setProductSearch('');
    setIsModalOpen(true);
  };
  ...
  const openEditModal = (deal: SpecialDeal) => {
    ...
    setFormHasSizes(Boolean(deal.hasSizes || (deal.sizes && (deal.sizes.small || deal.sizes.large))));
    setFormSizeSmallPrice(deal.sizes?.small || Math.round(deal.price * 0.75));
    setFormSizeMediumPrice(deal.sizes?.medium || deal.price);
    setFormSizeLargePrice(deal.sizes?.large || Math.round(deal.price * 1.35));
    setProductSearch('');
    setIsModalOpen(true);
  };
  ...
  const handleFormSubmit = async (e: React.FormEvent) => {
    ...
    setIsSaving(true);
    try {
      const dealPayload: Partial<SpecialDeal> & { name: string; price: number } = {
        id: editingDeal?.id,
        name: formName.trim(),
        description: formDescription.trim(),
        price: formHasSizes && formSizeSmallPrice ? Number(formSizeSmallPrice) : Number(formPrice),
        ...
        sizes: formHasSizes
          ? {
              small: Number(formSizeSmallPrice) || Number(formPrice),
              medium: Number(formSizeMediumPrice) || Math.round(Number(formPrice) * 1.3),
              large: Number(formSizeLargePrice) || Math.round(Number(formPrice) * 1.7)
            }
          : undefined
      };
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/85 backdrop-blur-sm overflow-y-auto">
          <div className="relative w-full max-w-2xl bg-[#2C0202] rounded-3xl border border-[#FFB703]/40 p-6 sm:p-8 space-y-6 shadow-2xl my-8 text-[#FFFBEB]">
            ...
            <form onSubmit={handleFormSubmit} className="space-y-4 text-xs">
              ...
              {/* SIZES CONFIGURATION FOR DEALS (Small, Medium, Large) */}
                ...
                <div className="flex items-center justify-between">
                  <div>
                    <span className="font-bold text-[#FFB703] text-xs uppercase block">
                      Deal Size Variants (Small / Medium / Large)
                    </span>
                    <p className="text-[11px] text-gray-400">
                      Enable if this deal has different prices for Small, Medium, or Large sizes
                    </p>
                  </div>
                  <label className="relative inline-flex items-center cursor-pointer">
                    <input
                      ...
                      onChange={(e) => setFormHasSizes(e.target.checked)}
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 pt-2 border-t border-white/10">
                    <div>
                      ...
                      <input
                        ...
                        value={formSizeSmallPrice}
                        onChange={(e) => setFormSizeSmallPrice(e.target.value === '' ? '' : Number(e.target.value))}
                        placeholder="1200"
                        className="w-full px-3 py-2 rounded-xl bg-[#2C0202] border border-[#FFB703]/30 text-white font-bold text-sm focus:outline-none focus:border-[#FFB703]"
                      />
                    </div>

                    <div>
                      <label className="block text-[11px] font-bold text-[#FFD166] mb-1">
                        Medium Deal Price (Rs.)
                      </label>
                      <input
                        type="number"
                        required={formHasSizes}
                        value={formSizeMediumPrice}
                        onChange={(e) => setFormSizeMediumPrice(e.target.value === '' ? '' : Number(e.target.value))}
                        placeholder="1650"
                        className="w-full px-3 py-2 rounded-xl bg-[#2C0202] border border-[#FFB703]/40 text-white font-bold text-sm focus:outline-none focus:border-[#FFB703]"
                      />
                    </div>
                    ...
                    <div>
                      ...
                      <input
                        type="number"
                        required={formHasSizes}
                        value={formSizeLargePrice}
                        onChange={(e) => setFormSizeLargePrice(e.target.value === '' ? '' : Number(e.target.value))}
                        placeholder="2100"
                        className="w-full px-3 py-2 rounded-xl bg-[#2C0202] border border-[#FFB703]/30 text-white font-bold text-sm focus:outline-none focus:border-[#FFB703]"
                      />
                    </div>
